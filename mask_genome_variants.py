...
def parse_args():
    ...
    parser.add_argument('-a', '--alternate-only', default=False, action='store_true', help='replace reference base with alternate allele, rather than retaining both (Default: false)')
    parser.add_argument('-u', '--update', default=False, action='store_true', help='Update an already masked genome with another VCF file (Default: false)')
    parser.add_argument('-r', '--region', type=parse_region, nargs="+", metavar="CHR:START-END", help="Name and range of contigs to mask (Default: entire genome)")
    parser.add_argument('-b', '--boundary', type=int, default=0, metavar="INT", help="Width of region boundary where both alleles are included")
